...
milsteinApprox=zeros(1,N_approx+1); %initialise the output vectors
milsteinApprox(1)=initialS; %assign the first value in each vector to initialS
...
for i=1:N_approx
    ...
    milsteinApprox(i+1)=milsteinApprox(i)+hApprox*mu*milsteinApprox(i)+sigma*milsteinApprox(i)*increment+0.5*sigma^2*milsteinApprox(i)*(increment^2-hApprox); %calculate next element in approximation
end
